...
lib <- c("reshape2", "raster", "remote", "lubridate", "doParallel", 
         "RColorBrewer", "Rsenal", "grid", "ggplot2", "dplyr")
...
source(paste0(ch_dir_repo, "ttestInterQuad.R"))
source(paste0(ch_dir_repo, "ttestIntraQuad.R"))
...
# split raster
fls_ndvi_wolndvi <- paste0(ch_dir_data, "gimms_ndvi3g_dwnscl_8211_wolndvi.tif")
...
fls_split <- list.files(paste0(ch_dir_data, "ul"), pattern = "dwnscl_8211.*.tif$", 
                        full.names = TRUE)
...
num_split_ltm <- foreach(i = rst_split, j = list("0_0", "0_1", "1_0", "1_1"), 
  ...
  rst <- stack(paste0(ch_dir_data, "ul/gimms_ndvi3g_dwnscl_8211_ltm_", j, ".tif"))
  mat <- as.matrix(rst)
